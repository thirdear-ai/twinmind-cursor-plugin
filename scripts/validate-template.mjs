...
async function main() {
  ...
  // Single-plugin repos keep the plugin at the root and omit marketplace.json
  // (per cursor.com/docs/reference/plugins); validate the root as the only plugin.
  const rootManifestPath = path.join(repoRoot, ".cursor-plugin", "plugin.json");
  const isSinglePlugin = !(await pathExists(marketplacePath)) && (await pathExists(rootManifestPath));
  const marketplace = isSinglePlugin
    ? await singlePluginMarketplace(rootManifestPath)
    : await readJsonFile(marketplacePath, "Marketplace manifest");
  if (!marketplace) {
    summarizeAndExit();
    return;
  }
...
async function singlePluginMarketplace(manifestPath) {
